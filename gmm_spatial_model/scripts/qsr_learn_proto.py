...
def draw_pose_arrow(ax, pose, arrow_length = 1, annotation = ''):
...
def pose_list_to_np(pose_list):
    ...
    """
    return np.array([[p.position.x, p.position.y] for p in pose_list])
...
if __name__ == '__main__':
    ...
    good_samples =  [mkpose(1.8, 1), mkpose(1.9, 2.1), mkpose(1.4, 0.5)]
    bad_samples = [mkpose(-2, 0.3), mkpose(-0.2, -0.5), mkpose(-2, 0.2)]

    good_samples = pose_list_to_np(good_samples)
    bad_samples = pose_list_to_np(bad_samples)
    ...
    draw_pose_arrow(ax, landmark_pose, arrow_length = 0.5)
